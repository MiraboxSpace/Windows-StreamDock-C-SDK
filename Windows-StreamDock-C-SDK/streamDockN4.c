...
// 声明需要重写的函数
//static streamDock* streamDockN4_init(streamDock* self, tranSport* transport, struct hid_device_info* devInfo);
//static void streamDockN4_destory(struct streamDock* self);
static int transform(int x);
static unsigned char* streamDockN4_getFirmVersion(struct streamDock* self, int lenth);
static int streamDockN4_setBrightness(struct streamDock* self, int percent);
static int streamDockN4_setBackgroundImg(struct streamDock* self, const char* path);
static int streamDockN4_setBackgroundImgData(struct streamDock* self, unsigned char* imagedata);
static unsigned char* streamDockN4_read(struct streamDock* self);
static void streamDockN4_readToVector(struct streamDock* self, unsigned char* vec);
static int streamDockN4_setKeyImg(struct streamDock* self, const char* path, int key);
static int streamDockN4_setKeyImgData(struct streamDock* self, unsigned char* imagedata, int key);
static int streamDockN4_clearIcon(struct streamDock* self, int index);

// 子类(streamDock293)全局变量, 析构函数不需要重写了, extern到外面后直接init就能用了
//streamDock streamDock293 = {
//    .init = streamDockN4_init,
//    .destory = streamDockN4_destory
//};
...
streamDock* streamDockN4_init(streamDock* self, tranSport* transport, struct hid_device_info* devInfo)
{
    ...
    // 构造子类
    self->init = streamDockN4_init;
    self->destory = streamDockN4_destory;         // 内部使用基类的析构函数
    self->getFirmVersion = streamDockN4_getFirmVersion;
    self->setBrightness = streamDockN4_setBrightness;
    self->setBackgroundImg = streamDockN4_setBackgroundImg;
    self->setBackgroundImgData = streamDockN4_setBackgroundImgData;
    self->read = streamDockN4_read;
    self->readToVector = streamDockN4_readToVector;
    self->setKeyImg = streamDockN4_setKeyImg;
    self->setKeyImgData = streamDockN4_setKeyImgData;
    return self;
}
...
// 返回值需要释放
static IplImage* rotate180_N4(IplImage* image, const char* temp_filename, int width, int height)
{
    //  复制图像，便于操作翻转
...
static void rotate180_N4_BG(IplImage* image, const char* temp_filename)
{
    // 旋转图像 180°
...
unsigned char* streamDockN4_getFirmVersion(streamDock* self, int lenth)
{
    if (!self) return NULL;
    return tranSportGetInputReport(self->transport, lenth);
}

int streamDockN4_setBrightness(streamDock* self, int percent)
{
    if (!self) return -1;
    return tranSportSetBrightness(self->transport, percent);
}

int streamDockN4_setBackgroundImg(streamDock* self, const char* path)
{
    if (!self) return -1;
    // 读取图像文件
    IplImage* image = cvLoadImage(path, CV_LOAD_IMAGE_COLOR);
    if (image == NULL) {
        fprintf(stderr, "streamDockN4_setBackgroundImg malloc error.\n");
        return -1;
    }
    ...
    // 旋转后发送
    const char* temp_filename = "N4_tmp_background.jpg";
    rotate180_N4_BG(image, temp_filename);
...
int streamDockN4_setBackgroundImgData(streamDock* self, unsigned char* imagedata)
{
    if (!self) return -1;
    int height = 480;
    int width = 800;
    ...
    if (!image) {
        fprintf(stderr, "streamDockN4_setBackgroundImgData malloc error.\n");
        return -1;
    }
    // 将入参的图像数据复制到 IplImage 中
    memcpy(image->imageData, imagedata, width * height * 3);
    // 旋转后发送
    const char* temp_filename = "N4_tmp_background_data.jpg";
    rotate180_N4_BG(image, temp_filename);
    int res = tranSportSetBackgroundImgDualDevice(self->transport, temp_filename);
    remove(temp_filename);
    // 释放资源
...
unsigned char* streamDockN4_read(streamDock* self)
{
    if (!self) return NULL;
    ...
    if (!command)
    {
        fprintf(stderr, "streamDockN4_readToVector malloc error...\n");
    }
...
void streamDockN4_readToVector(streamDock* self, unsigned char* vec)
{
    if (!self) return;
    if (!vec) return;
    // clear vec
...
int streamDockN4_setKeyImg(streamDock* self, const char* path, int key)
{
    if (!self) return -1;
    key = transform(key);
    ...
    // 旋转图片180度后发送
    return streamDockN4_setKeyImgData(self, image->imageData, key);
}

int streamDockN4_setKeyImgData(streamDock* self, unsigned char* imagedata, int key)
{
    if (!self) return -1;
    key = transform(key);
    int height = 112;
    ...
    const char* temp_filename = "N4_tmp_key.jpg";

    // 旋转180度后发送
    IplImage* backup_image = rotate180_N4(image, temp_filename, width, height);
...
int streamDockN4_clearIcon(streamDock* self, int index)
{
    if (!self) return -1;
